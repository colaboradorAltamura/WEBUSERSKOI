...
import { ApplicantsStateTypes, IApplicant, IHealthInsuranceAgreement, IUserBasicData } from 'src/types/@autogenerated';
...
// Se cambia
interface IHealthInsuranceAgreementRow extends IHealthInsuranceAgreement {
  showRow?: boolean;
}

interface CellType {
  row: IHealthInsuranceAgreementRow;
}
...
const AgreementsTab = ({ docId }: PropsType) => {
  // ** Hooks
  ...
  const [entitiesData, setEntitiesData] = useState<IHealthInsuranceAgreementRow[]>([]);
  ...
  useEffect(() => {
    const doAsync = async () => {
      try {
        ...
        // load entity data
        const agreementsResponse: any = await dynamicGet({
          params: '/cms/' + entitySchema.name + '/by-company/' + docId,
        });

        const agreements = agreementsResponse.items as IHealthInsuranceAgreement[];
        console.log(agreements);

        setEntitiesData(
          agreements.map((item: IHealthInsuranceAgreement) => {
            return {
              ...item,
              showRow: true,
            } as IHealthInsuranceAgreementRow;
          }) as IHealthInsuranceAgreementRow[]
        );
  ...
  const updateColumnsData = () => {
        ...
        <Box sx={{ display: 'flex', alignItems: 'center' }}>
          <Tooltip title='View Content'>
            <IconButton
              ...
              onClick={() => {
                router.push(`/agreements/${row.companyId}/${row.id}`);
              }}
            >
    ...
    columnsData.push({
      ...
      minWidth: 110,
      field: 'validFrom',
      headerName: t('valid from') as string,
    ...
    columnsData.push({
      ...
      minWidth: 110,
      field: 'validUntil',
      headerName: t('valid until') as string,
