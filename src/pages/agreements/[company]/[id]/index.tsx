...
import SummaryTab from 'src/views/companies/AgreementsSummaryTab';
...
const SCHEMA_NAME = CMSCollections.HEALTH_INSURANCE_AGREEMENTS;
...
const AgreementsEdit = ({}: InferGetStaticPropsType<typeof getStaticProps>) => {
  ...
  const [entityData, setEntityData] = useState<IHealthInsuranceAgreement | null>(null);
  ...
  useEffect(() => {
    ...
    const doAsync = async () => {
      try {
        ...
        const agreementResponse: any = await dynamicGet({ params: `/cms/${SCHEMA_NAME}/${id}` });
        const companyResponse: any = await dynamicGet({ params: `/cms/${COMPANY_SCHEMA}/${companyId}` });
        setEntityData(agreementResponse);
  ...
  const renderLeftPanel = () => {
    if (!entitySchema || !companyData) return;
        ...
        <Grid item xs={12}>
          <Card>
            ...
            <CardContent sx={{ pt: 13.5, display: 'flex', alignItems: 'center', flexDirection: 'column' }}>
              ...
              >
                {getInitials(capitalize(companyData.name))}
              </CustomAvatar>
            ...
            <CardContent sx={{ pb: 2 }}>
              <Box sx={{ pt: 2 }}>
                <Box sx={{ display: 'flex', mb: 3, margin: 2 }}>
                  ...
                  <Grid sx={{ ml: 2 }}>
                    <Typography sx={{ color: 'text.secondary' }}>{capitalize(companyData.contactName)}</Typography>
                  </Grid>
                </Box>
                <Box sx={{ display: 'flex', mb: 3, margin: 2 }}>
                  <Grid sx={{ ml: 2 }}>
                  ...
                  <Grid sx={{ ml: 2 }}>
                    {companyData.contactEmail ?? (
                      <Typography sx={{ color: 'text.secondary' }}>{companyData.contactEmail}</Typography>
                    )}
                  </Grid>
                </Box>
                <Box sx={{ display: 'flex', mb: 3, margin: 2 }}>
                  ...
                  <Grid sx={{ ml: 2 }}>
                    <Typography sx={{ color: 'text.secondary' }}>{companyData.contactPhone}</Typography>
                  </Grid>
                </Box>
                <Box sx={{ display: 'flex', mb: 3, margin: 2 }}>
