...
const WorkerProfile = () => {
  ...
  const handleTabChange = (event: SyntheticEvent, value: string) => {
    if (!currentUser.currentUser || !currentUser.currentUser.id) return;
    ...
    router
      .push({
        pathname: `/${SCHEMA_NAME}/${currentUser.currentUser.id}`,
        query: `tab=${value}`,
      })
  ...
  const handleOnEntityDelete = async () => {
    try {
      if (!entitySchema || !currentUser.currentUser || !currentUser.currentUser.id)
  ...
  const handleStateUpdate = async (targetState: WorkerStateTypes) => {
    try {
      if (!entityData) throw new Error('Missing entityData/currentUser.currentUser');
  ...
  const updateWorker = async (formData: any) => {
    ...
    response = await dynamicUpdate({
      params: `/cms/${SCHEMA_NAME}/` + currentUser.currentUser.id,
      data: formData,
    });
  };
  ...
  const renderRightPanel = () => {
    ...
    if (!activeTab || !entitySchema || !currentUser.currentUser?.id || !entityData) return null;
    ...
    return (
      <TabContext value={activeTab}>
        ...
        <TabPanel sx={{ p: 0 }} value={INITIAL_TAB}>
          <SummaryTab
            docId={currentUser.currentUser.id}
  ...
  if (router.isFallback || !currentUser || !currentUser.currentUser) {
