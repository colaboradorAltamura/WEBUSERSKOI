...
import SummaryTab from 'src/views/patients/SummaryTab';
...
const PatientWorkerEdit = ({}: InferGetStaticPropsType<typeof getStaticProps>) => {
  // ** Hooks
    ...
    const doAsync = async () => {
      try {
        ...
        const schema = dynamics.entitySchemas.find((schema) => {
          return schema.name === CMSCollections.PATIENTS;
        });
        if (!schema) throw new Error('Missing schemaName: ' + CMSCollections.PATIENTS);
  ...
  useEffect(() => {
    ...
    const doAsync = async () => {
      try {
        ...
        const data: IPatient = await dynamicGet({
          params: '/cms/' + CMSCollections.PATIENTS + '/' + id,
          //params: '/cms/' + PATIENT_WORKERS + '/mine/' + workerDataArg.id,
        });
...
export default PatientWorkerEdit;
